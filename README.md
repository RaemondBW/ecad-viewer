# cadence-viewer — Allegro `.brd` + OrCAD `.DSN` → self-contained HTML

Pure-Python converters that read Cadence's proprietary binary formats directly —
no Cadence tools, no exports — and emit single-file interactive HTML viewers:

- **Schematic** (`orcad_viewer.py`): OrCAD Capture `.DSN` → multi-sheet schematic
  with native symbol geometry, net highlighting, part cards, search, and an
  optional revision **diff** against an older `.DSN`.
- **Layout** (`brd_viewer.py`): Allegro `.brd` → board view with per-layer copper,
...
| File | Role |
|---|---|
| `orcad_convert.py` | `.DSN` parser (OLE2 container → design, sheets, parts, wires, ports) |
| `orcad_viewer.py`  | schematic model builder + HTML template; `build_model()`, `generate()` |
...
| `docs/orcad-dsn-format.md`   | reverse-engineered `.DSN` format reference |
| `docs/allegro-brd-format.md` | reverse-engineered `.brd` format reference |
...
#   → out/orcad_schematic.html and out/pcb.html

# a single view:
python3 orcad_viewer.py board.DSN -o schematic.html [--diff old.DSN]
...
The `.BOM` is optional: an OrCAD Bill-of-Materials text export that supplies
...
```python
import orcad_viewer, brd_viewer, xprobe

model = orcad_viewer.build_model("board.DSN", diff_path=None)   # dict; JSON-serialisable
orcad_viewer.generate("board.DSN", "schematic.html")
