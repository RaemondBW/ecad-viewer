  ...
  if (!XMODAL) {
    Comments.init({
      ...
      resolveAnchor: (cx, cy) => {
        ...
        for (const p of s.parts) for (const pin of (p.pins || [])) { const dx = pin[0] - sx, dy = pin[1] - sy, d = dx * dx + dy * dy; if (d < bd) { bd = d; best = { kind: 'pin', tx: pin[0], ty: pin[1], ref: p.des + '.' + pin[3], label: 'Pin ' + p.des + '.' + pin[3] }; } }
